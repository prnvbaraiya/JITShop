              ...
              <table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="20" width="100%">
                ...
                <tbody>
                  ...
                  @for($i=0;$i<count($columns)-2;$i++)
                      <td style="max-width: 300px;" data-toggle="tooltip" title="{{$content[$columns[$i]]}}" class="py-1 text-truncate">
                          {{$content[$columns[$i]]}}
                      </td>
                    @endfor
                      <td class="th-sm"><a href="/admin/{{$tableName}}/{{$content['id']}}"><button class="btn btn-primary">Edit</button></a>
                      </td>
                      <td class="th-sm"><a href="/admin/{{$tableName}}/delete/{{$content['id']}}"><button class="btn btn-danger">Delete</button></a>
                      </td>
                    </tr>
                   @endforeach
                  </tbody>
