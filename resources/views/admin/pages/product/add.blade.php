...
<div class="col-12 grid-margin">
  <div class="card">
    <div class="card-body">
      ...
      <form action="/admin/product" enctype="multipart/form-data" class="form-sample" method="post">
          @csrf
          <div class="row">
                ...
                <script>
                    ...
                    function remove(){
                        ...
                        @for($i=0;$i<count($attributes);$i++){
                            ...
                            {
                                content+='{{$attributes[$i]->name}}</label><div class="m-2 align-items-center col-sm-7 d-flex flex-wrap">';
                                <?php $j=0?>
                                @foreach(explode(',', $attributes[$i]->value) as $value)
                                    content+='<input type="checkbox" class="me-1" name="attributeArr[{{$attributes[$i]->id}}][]" value="{{$j++}}"><label class="me-3" for="{{$value}}">{{$value}}</label>';
                                @endforeach
                                content+='</div></div>';
                                $(".main").append(content);
                                content='';
                        ...
                        var data = '<div class="col-sm-6 d-flex removeAttributes"><select id="attribute_id" name="attribute_id" class="form-control">';
                        @foreach($attributes as $attribute)
        ...
        <div class="row">
            <div class="col-md-12 mt-4">
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <textarea type="text" value="{{old('details')}}" name="details" class="form-control" placeholder="Enter Product Details Here..." >{{old('details')}}</textarea>
                    </div>
                </div>
            </div>
        </div>
        @error('details')<div class="error text-danger">{{ $message }}</div>@enderror
        <div class="row">
            <div class="col-md-12 mt-4">
                <div class="form-group row">
                    <label class="col-sm-3 col-form-label">Product Price</label>
                    <div class="col-sm-3">
                        <input type="text" value="{{old('price')}}" name="price" class="form-control" placeholder="Enter Price" />
                    </div>
                    @error('price')<div class="error text-danger">{{ $message }}</div>@enderror
                    <label class="col-sm-3 col-form-label">Product Quantity</label>
                    <div class="col-sm-3">
                        <input type="text" name="quantity" value="{{old('quantity')}}" class="form-control" placeholder="Enter Quantity" />
                    </div>
                </div>
            @error('quantity')<div class="error text-danger">{{ $message }}</div>@enderror
            </div>
        </div>
